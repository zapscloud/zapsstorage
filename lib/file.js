...
module.exports = class File {
    ...
    uploadFile(folderpath, fullfilepath, ispublic) {
        return new Promise((resolve, reject) => {
            ...
            fs.readFile(path.join(fullfilepath), (err, fileData) => {
                ...
                axios.post(`https://storage.${this.stage}api.zapscloud.com/${_path}`, formData, this.header_param)
    ...
    removeFile(fullfilepath) {
        return new Promise((resolve, reject) => {
            ...
            axios.delete(`https://storage.${this.stage}api.zapscloud.com/${_path}`, this.header_param)
                .then(function (response) {
                    return resolve(response.data);
                })
                .catch(function (error) {
    ...
    getFileDetail(fullfilepath) {
        return new Promise((resolve, reject) => {
            var _path = `${fullfilepath}`.replace(new RegExp('//', 'g'),'/')
            axios.get(`https://storage.${this.stage}api.zapscloud.com/${_path}?action=stat`, this.header_param)
                .then(function (response) {
                    return resolve(response.data);
                })
                .catch(function (error) {
    ...
    getFile(fullfilepath, downloadpath) {
        return new Promise((resolve, reject) => {
            ...
            var _path = `${fullfilepath}`.replace(new RegExp('//', 'g'),'/')
            var _url = `https://storage.${this.stage}api.zapscloud.com/${_path}`;
    ...
    getFileList(folderpath) {
        return new Promise((resolve, reject) => {
            ...
            axios.get(`https://storage.${this.stage}api.zapscloud.com/${_path}?action=list`, this.header_param)
                .then(function (response) {
                    return resolve(response.data);
                })
                .catch(function (error) {
