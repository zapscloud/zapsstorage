...
module.exports = class File {
    ...
    uploadFile(folderpath, fullfilepath, ispublic) {
        return new Promise((resolve, reject) => {
            ...
            fs.readFile(path.join(fullfilepath), (err, fileData) => {
                ...
                var _path = `/storages/${folderpath}${_ispulbic}`.replace(new RegExp('//', 'g'),'/')
                axios.post(`${this.zapsurl}${_path}`, formData, this.header_param)
    ...
    removeFile(fullfilepath) {
        return new Promise((resolve, reject) => {
            var _path = `/storages/${fullfilepath}`.replace(new RegExp('//', 'g'),'/')
            axios.delete(`${this.zapsurl}${_path}`, this.header_param)
                .then(function (response) {
                    return resolve(response.data);
                })
                .catch(function (error) {
    ...
    getFileDetail(fullfilepath) {
        return new Promise((resolve, reject) => {
            var _path = `/storages/${fullfilepath}`.replace(new RegExp('//', 'g'),'/')
            axios.get(`${this.zapsurl}${_path}?action=stat`, this.header_param)
                .then(function (response) {
                    return resolve(response.data);
                })
                .catch(function (error) {
    ...
    getFileList(folderpath) {
        return new Promise((resolve, reject) => {
            var _path = `/storages/${folderpath}`.replace(new RegExp('//', 'g'),'/')
            axios.get(`${this.zapsurl}${_path}?action=list`, this.header_param)
                .then(function (response) {
                    return resolve(response.data);
                })
                .catch(function (error) {
